...
class ToolExecutor:
    ...
    def execute_tool_calls(self, assistant_message_content: list) -> tuple[list[dict], list[str]]:
        ...
        tool_use_blocks = [block for block in assistant_message_content if block.get("type") == "tool_use"]
    ...
    def _execute_single_tool_call(self, tool_block: dict) -> dict:
        ...
        try:
            ...
            if not isinstance(tool_input, dict):
                raise TypeError(f"Tool input for '{tool_name}' must be a dictionary, not {type(tool_input).__name__}.")
    ...
    def _format_result(self, tool_id: str, content: Any, is_error: bool = False) -> dict:
        ...
        try:
            ...
            content_str = f"Error: Could not serialize tool output of type {type(content).__name__}."
            is_error = True
